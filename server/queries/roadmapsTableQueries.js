...
async function queryRoadmaps() {
  try {
    let roadmaps = await (
      await pool.query('SELECT id AS id, title, value FROM roadmaps_new')
    ).rows;
    return roadmaps;
  } catch (e) {
    console.log(e);
...
async function queryToAddRoadmap(newRoadmap) {
  ...
  console.log('type of ', typeof newRoadmap.stages);
  const response = await pool.query(
    `INSERT INTO roadmaps_new (id, title, value) VALUES('${newRoadmap.id}', '${
      newRoadmap.title
    }', '${JSON.stringify(newRoadmap.stages)}')`
  );
  return response;
}
